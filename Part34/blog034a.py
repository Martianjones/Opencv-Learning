...
def LocateLicense(img,sourceimg):
    ...
    for i in range(len(block)):
        b = sourceimg[block[i][0][1]:block[i][0][3],block[i][0][0]:block[i][0][2]]
        # BGR 转 HSV
...
def ShowImage(img,gray,stretchedimg,openimg,diffimg,binary,canny,openimg3,result):
    titles = ['Source Image','Gray Image','Stretched Image','Opening Image','Diff Image','Binary Image',
              'Canny Image','Locate Image','Result Image']
    images = [img,gray,stretchedimg,openimg,diffimg,binary,canny,openimg3,result]
    for i in range(9):
